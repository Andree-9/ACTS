...
class WandbLogger:
    ...
    def __init__(self, args) -> None:
        ...
        # Track explicitly-defined metric keys (including nested paths) to ensure
        # they all use the intended x-axis metric.
        self._defined_train_metrics = set()
    ...
    def log_train(self, global_step: int, logs_dict: Dict[str, Any]) -> None:
        ...
        if generated_samples:
            ...
            generated_samples_table = new_table

        metrics = {k: v for k, v in logs_dict.items() if v is not None}

        # Explicitly bind each metric key to train/global_step so nested keys
        # like train/controller/* use the same x-axis as train/reward.
        for key in metrics:
            metric_name = f"train/{key}"
            if metric_name not in self._defined_train_metrics:
                self.handle.define_metric(metric_name, step_metric="train/global_step", step_sync=True)
                self._defined_train_metrics.add(metric_name)

        logs = {"train/%s" % k: v for k, v in {**metrics, "global_step": global_step}.items()}
        if generated_samples_table is not None:
            logs["train/generated_samples"] = generated_samples_table
        # Log once per global step to avoid accidental extra implicit _step bumps.
        self.handle.log(logs, step=global_step)
