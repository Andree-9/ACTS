...
class BasePPOTrainer(ABC):
    ...
    def _compute_rollout_reward_stats(self, rollout_samples) -> Dict[str, float]:
        """
        Compute raw reward dispersion metrics from rollout samples for logging.

        Metrics are calculated before PPO updates so they reflect rollout quality
        and GRPO grouping signal strength directly.
        """
        reward_values = []
        for sample in rollout_samples:
            ...
            if isinstance(rewards, torch.Tensor):
                if rewards.numel() == 0:
                    continue
                reward_values.append(float(rewards.reshape(-1)[0].item()))
            else:
                reward_values.append(float(rewards))
        ...
        stats: Dict[str, float] = {
            "rollout_reward_mean": rewards_tensor.mean().item(),
            "rollout_reward_std": rewards_tensor.std(unbiased=False).item(),
            "rollout_reward_max": rewards_tensor.max().item(),
            "rollout_reward_min": rewards_tensor.min().item(),
        }

        n_samples_per_prompt = int(getattr(self.args, "n_samples_per_prompt", 1))
        if n_samples_per_prompt > 1 and rewards_tensor.numel() % n_samples_per_prompt == 0:
            grouped_rewards = rewards_tensor.reshape(-1, n_samples_per_prompt)
            group_reward_std = grouped_rewards.std(dim=-1, unbiased=False)
            group_reward_max = grouped_rewards.max(dim=-1).values
            group_reward_min = grouped_rewards.min(dim=-1).values
            stats["rollout_group_reward_std"] = group_reward_std.mean().item()
            stats["rollout_group_reward_max"] = group_reward_max.mean().item()
            stats["rollout_group_reward_min"] = group_reward_min.mean().item()

        return stats

    def train_step(self, rollout_samples, global_step: int) -> Tuple[Dict, int]:
        rollout_reward_stats = self._compute_rollout_reward_stats(rollout_samples)
        ...
        status.update(rollout_reward_stats)
