    ...
    def add_slime_arguments(parser):
        ...
        def add_rollout_arguments(parser):
            ...
            parser.add_argument(
                ...
                default=False,
                help=(
                    "Use the dynamic sampling filter as a ranking preference instead of a hard rejection loop. "
                    "One over_sampling_batch_size wave is generated, groups that pass the filter are selected first, "
                    "and any remaining rollout_batch_size slots are filled with filtered groups."
                ),
            )
        ...
        def add_algo_arguments(parser):
            ...
            parser.add_argument(
                ...
                default=None,
                help=(
                    "Maximum number of full Megatron iter_* checkpoints to keep under --save. "
                    "Older checkpoint directories and matching rollout dataset state files are pruned after a successful save. "
                    "Set to None to keep all checkpoints."
                ),
            )
            parser.add_argument(
                "--save-hf",
            ...
            parser.add_argument(
                ...
                default=False,
                help=(
                    "Only save HuggingFace format checkpoints (model weights only, ~8GB for 4B model) "
                    "instead of full Megatron distributed checkpoints (weights + optimizer + fp32 copies, ~53GB). "
                    "Significantly reduces disk usage but disables training resumption from saved checkpoints. "
                    "Requires --save-hf to be set, or auto-derives it from --save."
                ),
            )
...
def slime_validate_args(args):
    ...
    if args.kl_coef != 0 or args.use_kl_loss:
        if args.ref_load is None:
            raise ValueError("--ref-load must be set when --kl-coef is nonzero or --use-kl-loss is enabled.")
        ref_load_is_hf_bridge = (
            args.megatron_to_hf_mode == "bridge"
            and args.ref_load is not None
            and "/" in str(args.ref_load)
            and not str(args.ref_load).startswith(("/", "."))
        )
        if not ref_load_is_hf_bridge and not os.path.exists(args.ref_load):
            raise FileNotFoundError(f"ref_load {args.ref_load} does not exist, please check the path.")

        if not ref_load_is_hf_bridge and not os.path.exists(os.path.join(args.ref_load, "latest_checkpointed_iteration.txt")):
            logger.info(
    ...
    if args.max_checkpoints_to_keep is not None:
        assert args.max_checkpoints_to_keep > 0, "--max-checkpoints-to-keep must be positive."

    if args.save_hf_only:
        if args.save_hf is None:
            assert args.save is not None, "'--save' or '--save-hf' is required when --save-hf-only is set."
            args.save_hf = os.path.join(args.save, "hf", "step_{rollout_id}")
        assert args.hf_checkpoint is not None, (
            "'--hf-checkpoint' is required for --save-hf-only (needed by AutoBridge for HF format conversion)."
        )
