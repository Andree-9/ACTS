...
def start_rollout_servers(args, pg) -> dict[str, RolloutServer]:
    ...
    for model_idx, model_cfg in enumerate(config.models):
        ...
        def _make_group(group_cfg, router_ip, router_port, overrides_extra=None):
            ...
            if needs_offload and not model_cfg.update_weights:
                # Frozen colocated models are restored via resume_memory_occupation
                # before rollout. Keep a CPU weight backup so their first decode
                # after onload does not depend on a stale/corrupted weight state.
