...
class MegatronTrainRayActor(TrainRayActor):
    ...
    def sleep(self) -> None:
        ...
        # Reclaim host memory left behind by NCCL communicator teardown
        # before torch_memory_saver allocates its CPU backup.
        clear_memory(clear_host_memory=True)
    ...
    def update_weights(self) -> None:
        ...
        if self.args.offload_train:
            destroy_process_groups()
            # Reclaim host memory fragmented by NCCL communicator teardown.
            # Without this, repeated PG create/destroy cycles leak pinned host
            # memory (~hundreds of KB each) that accumulates over long runs.
            clear_memory(clear_host_memory=True)
