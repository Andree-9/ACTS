...
def load_checkpoint(ddp_model, optimizer, opt_param_scheduler, checkpointing_context, skip_load_to_model_and_opt):
    ...
    load_path_exists = Path(load_path).exists() and _is_dir_nonempty(load_path)
    if not load_path_exists:
        if args.megatron_to_hf_mode == "bridge" and _looks_like_hf_repo_id(load_path):
...
def _looks_like_hf_repo_id(path: str | Path | None) -> bool:
    if path is None:
        return False
    value = str(path)
